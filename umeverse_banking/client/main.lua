...
CreateThread(function()
    while true do
        ...
        if UME.IsLoggedIn() and not UME.IsDead() then
            ...
            -- ATM interaction (using world ATM props)
            if not nearBank then
                local atm = GetClosestObjectOfType(myCoords.x, myCoords.y, myCoords.z, 1.5, GetHashKey('prop_atm_01'), false, false, false)
                if atm == 0 then
                    atm = GetClosestObjectOfType(myCoords.x, myCoords.y, myCoords.z, 1.5, GetHashKey('prop_atm_02'), false, false, false)
                end
                if atm == 0 then
                    atm = GetClosestObjectOfType(myCoords.x, myCoords.y, myCoords.z, 1.5, GetHashKey('prop_atm_03'), false, false, false)
                end
                if atm == 0 then
                    atm = GetClosestObjectOfType(myCoords.x, myCoords.y, myCoords.z, 1.5, GetHashKey('prop_fleeca_atm'), false, false, false)
                end
